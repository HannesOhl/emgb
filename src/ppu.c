...
static uint8_t bg_pixel(Ppu* ppu, uint32_t x, uint32_t y) {
	...
	int32_t gx = (x + scx) & 0xFF;
	int32_t gy = (y + scy) & 0xFF;
	...
	uint8_t tile_index = bus_read(ppu->bus, (uint16_t) (map_base - 0x8000) + map_offset);
	...
	uint8_t low  = bus_read(ppu->bus, (uint16_t) (tile_addr - 0x8000 + row * 2));
	uint8_t high = bus_read(ppu->bus, (uint16_t) (tile_addr - 0x8000 + row * 2 + 1));
...
void ppu_step(Ppu* ppu, uint32_t cycles) {
	...
	while (ppu->dots >= 456) {
		...
		// v_blank
		if (ppu->ly == 144) bus_write(ppu->bus, 0xFF0F, bus_read(ppu->bus, 0xFF0F) | 0x01);
